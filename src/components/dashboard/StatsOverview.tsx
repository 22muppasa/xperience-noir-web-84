...
const StatsOverview = () => {
  ...
  const { data: stats, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [messagesResult, notificationsResult, kidsWorkResult] = await Promise.all([
        supabase
          .from('messages')
  ...
  if (error) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="col-span-3 text-center p-8 bg-white border border-black rounded-lg">
